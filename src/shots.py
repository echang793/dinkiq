...
from court import COURT_L, NET_Y, dist_from_net
...
def serve_metrics(rallies: list[dict], subject_hits: np.ndarray,
    ...
    """Serve depth from the first bounce after a subject-served rally start.
    ...
    for r in rallies:
        ...
        depths.append(round(float(abs(baseline_y - first["y"])), 1))
    if not depths:
        return {"serves_measured": 0}
    return {
        "serves_measured": len(depths),
...
def shot_report(hit_times: np.ndarray, subject_hits: np.ndarray, ball: pd.DataFrame,
    ...
    for t, mine in zip(hit_times, subject_hits):
        ...
        shots.append({"t": round(float(t), 2),
